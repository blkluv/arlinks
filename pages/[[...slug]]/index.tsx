...
export default function Page() {
  ...
  useEffect(() => {
    ...
    // urlSlugToState()
    (async() => {
      console.log("Running useeffect async thing")
      if (slug && !pageState.address.value) {
        console.log("True: (slug && !pageState.address.value)")
        const b64Pattern = /^[A-Za-z0-9\-_]*={0,2}$/;
        console.log(`Slug regex tester: ${b64Pattern.test(slug)}`)
        const slugIsNewTxId: boolean = typeof slug !== 'undefined' && slug.length === 43 && b64Pattern.test(slug);
        console.log(`slugIsNewTxId = ${JSON.stringify(slugIsNewTxId)}`)
        if (slugIsNewTxId) {
          pageState.address.set(slug);
